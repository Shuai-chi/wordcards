  ...
  getCardsByDeck: async (deckId: string): Promise<Card[]> => {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        req.onsuccess = () => resolve(req.result);
        req.onerror = () => reject(req.error || new Error('Failed to fetch cards'));
      } catch (err) {
        reject(err);
      }
    });
  ...
  putCards: async (cards: Card[]): Promise<void> => {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        tx.oncomplete = () => resolve();
        tx.onerror = () => reject(tx.error || new Error(`Failed to save ${cards.length} cards (check storage quota)`));
        tx.onabort = () => reject(tx.error || new Error(`Database transaction aborted while saving cards (check storage quota or invalid data)`));
      } catch (err) {
        reject(err);
      }
    });
  ...
  getTodayReport: async (todayStr: string = getTodayStr()): Promise<Report> => {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        req.onerror = () => reject(req.error || new Error('Failed to fetch today report'));
      } catch (err) {
        reject(err);
      }
    });
